Šī nodaļa palīdz saprast kopējo atmiņas organizāciju un kaudzes\footnote{Šī termina nozīme atšķiras no datu struktūras "kaudze", kurā elementi tiek izvēlēti saskaņā ar to prioritāti.} lomu. 
Šajā nodaļā ir aprakstīti atmiņas iedalīšanas paņēmieni, ir dots īss ieskāts GNU C bibliotēkas ptmalloc2 realizācijā un aprakstīta dinamiskās atmiņas pārvaldība.
...
Eksistē divi atmiņas iedalīšanas paņēmieni: statiskā un dinamiskā atmiņas iedalīšana (sk. 1.1. attēlu). 
...
Kontrolētā atmiņas iedalīšana nozīme, ka programma izvēlās brīvus atmiņas gabalus no pieejamas segmenta telpas, priekš programmas datiem. 
...
Svarīgākie procesa segmenti  \cite {atparv}:
%\vspace{-20 pt}
\begin{itemize}
...
Turpmāk tiks apskatīta lietotāja atmiņas pārvaldība no lietotnes, izmantojot GNU C bibliotēkas funkciju palīdzību.
C valodā dinamiskā atmiņa tiek pārvaldīta ar malloc(), realloc(), free() un calloc() funkciju palīdzību \cite {atparv}.
C++ valodā ir izmantots operators new, lai pieprasītu atmiņu. 
Attēlā 1.2. ir redzama C un C++ sintakse atmiņas pieprasīšanai izmantojot C un C++ kodu.
...
new un delete operatorus (sk. 1.3. attēlu), bet priekš C programmām malloc() un free().
...
\section{Dinamiskās atmiņas organizācija glibc bibliotēkā}
...
Atmiņas arēnu var nosaukt par loģisko atmiņas kolekciju. Attēlā\footnote{Attēla izveidošanai tika izmantots GNU C malloc pirmkods \cite {MALLOC} un vietnē nopublicēta shēma \cite{AMM}. Attēls demonstrē atmiņas organizāciju.} 1.4. ir parādītas 3 arēnas, kuras ir atdalītas savā starpā ar raustītam līnijām.
...
Gadījumos, kad pavedieni izmanto atmiņu no vairākām atsevišķām arēnām, piemēram kā tas notiek ptmalloc2 realizācijā, tad vienas arēnas bloķēšana neietekmē atmiņas iedalīšanu parējās kaudzēs, kuras nepieder nobloķētai arēnai, un atmiņas iedalīšana var notikt paralēli.
...
Priekš sarakstiem nav iedalīta atmiņa, bet tiek izmantota kaudzes atmiņa, pierakstot katram gabalam rādītājus.
...
Pēc noklusējuma ātro atmiņas gabalu izmērs nepārsniedz 64 baitus (sk. 1.5. attēlu), bet to var palielināt līdz 80 baitiem \cite {MALLOC}. 
...
Atmiņa no kaudzēs tiek iedalīta, izmantojot malloc\_chunk struktūru (sk. 1.6. attēlu). 
...
No kreisās pusēs  attēlots (sk. 1.7. attēlu) \cite{CHOFM} atmiņas gabals, kurš bija iedalīts procesam, no labās, tās, kurš bija atbrīvots.
